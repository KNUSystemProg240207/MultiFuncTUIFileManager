...
int readProcInfo(ProcWin *procWindow) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        */
        ...
        if (fscanf(statFile, "%d (%255[^)]) %c %*d %*d %*d %*d %*d %*u %*u %*u %*u %*u %lu %lu %*d %*d %*d %*d %*d %*d %*u %lu", &tempProc.pid, tempProc.name, &tempProc.state, &tempProc.utime, &tempProc.stime, &tempProc.vsize) != 6) {
            fclose(statFile);
            continue;
        }
